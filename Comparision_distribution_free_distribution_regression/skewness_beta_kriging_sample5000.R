...
cov_matrix_function_ML = function(theta,mq) { theta[1] * exp( - mDist__^(2*theta[3])  / theta[2]) + theta[4]*diag(dim(mq)[1]) }
...
name="skewness_beta_kriging_sample5000"
...
mDist__ = wasserstein_matrix_quantile( mq_obs , mq_obs)
y_obs=c(ytraining,yvalidation)

res_ML = maximum_likelihood(init_theta=init_theta,theta_inf=theta_inf,theta_sup=theta_sup,
                               ind_active=ind_active,nugget=nugget,cov_matrix_function=cov_matrix_function_ML,
