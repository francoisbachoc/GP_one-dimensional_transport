...
ind_active=seq(from=1,to=order+2,length=order+2)
d = 100
nugget=10^(-6)
f_code = f_code_cos
...
generate_pdf = generate_pdf_matern
param_pdf = c(2,0.4,3/2,0.5)
...
m_input_kernel_obs = from_pdf_to_vector_legendre(m_pdf_obs,order)
...
m_input_kernel_pred = from_pdf_to_vector_legendre(m_pdf_pred,order)
