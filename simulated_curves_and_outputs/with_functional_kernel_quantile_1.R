...
#######################################################
seed=1
name="res_functional_kernel_quantile"
...
res_ML = maximum_likelihood(init_theta=init_theta,theta_inf=c(sigma_inf,lc_inf,H_inf),theta_sup=c(sigma_sup,lc_sup,H_sup),
                               ...
                               mq_obs=m_q_obs,y_obs=y_obs,k=n_gradient_descent)  
...
R_obs = cov_matrix_exp_quantile(sigma,lc,H,m_q_obs,m_q_obs)
R_pred = cov_matrix_exp_quantile(sigma,lc,H,m_q_pred,m_q_pred)
R_obs_pred = cov_matrix_exp_quantile(sigma,lc,H,m_q_obs,m_q_pred)
